...
class SaleItemSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['pkg_sub'] = self.pkg_sub_build_field(required=False, context=context)
    ...
    def service_build_field(self, *args, **kwargs):
        if self.context['request'].method in ['PUT', 'PATCH', 'POST']:
            kwargs.pop('context')
            return serializers.PrimaryKeyRelatedField(queryset=Service.objects.all(), required=False, write_only=True, *args, **kwargs)
        else:
    ...
    def prod_build_field(self, *args, **kwargs):
        if self.context['request'].method in ['PUT', 'PATCH', 'POST']:
            kwargs.pop('context')
            return serializers.PrimaryKeyRelatedField(queryset=Product.objects.all(), required=False, write_only=True, *args, **kwargs)
        else:
            return ProductSerializer(read_only=True, *args, **kwargs)
        
    def pkg_sub_build_field(self, *args, **kwargs):
        return PackageSubscriptionSerializer(*args, **kwargs)
...
class SaleItemSaleSerializer(SaleSerializer):
...
class PackageSubscriptionSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Check if the serializer is used as a nested serializer
        if isinstance(self._context.get('parent'), SaleItemSaleSerializer):
            # Remove cust field as the value can be obtained automatically from parent serializer
            self.fields.pop('cust', None)
        elif isinstance(self._context.get('parent'), SaleItemSerializer) and self.context['request'].method not in ['GET']:
